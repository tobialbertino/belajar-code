...
public:
    ...
    void doIfKeyPressed(int key, int action){ // Arrow Key interaction 
    ...
    void doIfClicked(int button, int action, double xpos, double ypos){
        if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS && isRectangleSelected(xpos,ypos) && hold == false){
            hold = true;
        }
        // object Fall to y = 400
        if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_RELEASE && isRectangleSelected(xpos,ypos) && hold == true){
            hold = false;
            y = 400;
        }
    }
